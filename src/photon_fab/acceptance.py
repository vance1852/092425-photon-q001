...
def run() -> dict:
    ...
    for seq, (wavelength, response) in enumerate(((450, .71), (520, .93), (650, .84)), start=1):
        service.add_measurement(
            token, "LOT-DEMO", wavelength, response, .01, "spectrometer-1",
            measurement_no=f"MEAS-{seq:04d}",
        )
